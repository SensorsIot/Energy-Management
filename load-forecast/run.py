...
logger = logging.getLogger("load-forecast")
...
def load_options(config_path: str = None) -> dict:
    ...
    Strategy:
    1. Load defaults from /usr/share/load-forecast/load-forecast.yaml.example
    2. Load user config from /config/load-forecast.yaml (via --config)
    ...
    defaults_path = Path("/usr/share/load-forecast/load-forecast.yaml.example")
    if defaults_path.exists():
        logger.debug(f"Loading defaults from {defaults_path}")
        with open(defaults_path) as f:
            defaults = yaml.safe_load(f) or {}

    # Load user config (non-secrets from /config/load-forecast.yaml)
    if config_path:
        ...
        legacy_paths = [
            Path("/config/load-forecast.yaml"),
            Path("/share/load-forecast/config.yaml"),
        ]
