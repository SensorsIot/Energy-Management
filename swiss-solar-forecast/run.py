...
logger = logging.getLogger("swiss-solar-forecast")
...
class SwissSolarForecast:
    ...
    def __init__(self, options: dict) -> None:
        ...
        self.data_dir = Path(storage_config.get("data_path", "/share/swiss-solar-forecast"))
    ...
    def calculate_forecast(self) -> None:
        ...
        try:
            ...
            if self.influx_writer:
                ...
                # (energy-manager shaving) treat a stale heartbeat as "don't
...
def load_options(config_path: str = None) -> dict:
    ...
    Strategy:
    1. Load defaults from /usr/share/swiss-solar-forecast/swiss-solar-forecast.yaml.example
    2. Load user config from /config/swiss-solar-forecast.yaml (via --config)
    ...
    defaults_path = Path("/usr/share/swiss-solar-forecast/swiss-solar-forecast.yaml.example")
    if defaults_path.exists():
        logger.debug(f"Loading defaults from {defaults_path}")
        with open(defaults_path) as f:
            defaults = yaml.safe_load(f) or {}

    # Load user config (non-secrets from /config/swiss-solar-forecast.yaml)
    if config_path:
        ...
        legacy_paths = [
            Path("/config/swiss-solar-forecast.yaml"),
            Path("/share/swiss-solar-forecast/config.yaml"),
        ]
