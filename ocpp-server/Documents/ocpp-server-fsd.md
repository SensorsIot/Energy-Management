...
| SEC-04 | An unexpected charge-point id connects while the trusted wallbox has an active transaction | The stray connection cannot hijack, stop, or corrupt the active transaction/state | ASVS V2/V4, CWE-287 | 3.1 L | ✅ Decided — accept-any, per the trusted-LAN posture ([design-principles §7](../../Harness/project/design-principles.md)); severity downgraded (LAN-trusted). Optional future hardening: guard an active transaction against a duplicate connection |
...
All eight cases are now resolved: **SEC-01/02/03/06 built**, **SEC-05/07 pinned by tests**, and
**SEC-04/05/08 decided** by the trusted-LAN security posture
