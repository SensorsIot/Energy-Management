...
5. Various excess values → correct phase selection
6. Edge cases (rounding, custom limits)
"""
...
class TestPause:
    ...
    def test_no_excess_negative(self):
        """Negative excess -1000W → pause."""
...
class TestOnePhase:
    ...
    def test_excess_in_gap_clamps_to_1phase_max(self):
        ...
        result = calculate_ev_power(excess_w=4000)
        assert result.target_power_w == 3700
        assert "1-phase" in result.reason

    def test_excess_just_below_3phase(self):
        """Excess 4099W → 1-phase clamped at 3700W."""
        result = calculate_ev_power(excess_w=4099)
        assert result.target_power_w == 3700
        assert "1-phase" in result.reason
...
class TestThreePhase:
    ...
    def test_minimum_3phase(self):
        ...
        result = calculate_ev_power(excess_w=4100)
...
class TestExcessValues:
    """Verify correct results for various excess values."""

    def test_large_excess_3phase(self):
        """Excess 7000W → 3-phase at 7000W."""
        result = calculate_ev_power(excess_w=7000)
    ...
    def test_medium_excess_3phase(self):
        """Excess 5000W → 3-phase at 5000W."""
        result = calculate_ev_power(excess_w=5000)
        assert result.available_excess_w == 5000
        assert result.target_power_w == 5000
        assert "3-phase" in result.reason

    def test_gap_excess_1phase(self):
        """Excess 4000W (in gap) → 1-phase at 3700W."""
        result = calculate_ev_power(excess_w=4000)
        assert result.available_excess_w == 4000
        assert result.target_power_w == 3700
        assert "1-phase" in result.reason

    def test_negative_excess_pause(self):
        """Negative excess → pause."""
        result = calculate_ev_power(excess_w=-1000)
        assert result.available_excess_w == -1000
        assert result.target_power_w == 0

    def test_moderate_excess_3phase(self):
        """Excess 5000W → 3-phase at 5000W."""
        result = calculate_ev_power(excess_w=5000)
        assert result.available_excess_w == 5000
        assert result.target_power_w == 5000
...
class TestRounding:
    ...
    def test_rounding_does_not_exceed_max(self):
        ...
        result = calculate_ev_power(excess_w=3750)
        assert result.target_power_w == 3700
