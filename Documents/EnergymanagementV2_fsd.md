...
See the [SwissSolarForecast FSD](../swisssolarforecast/Documents/swisssolarforecast-fsd.md) §7
(Configuration) for this add-on's full parameter reference.
...
SwissSolarForecast is an independent Home Assistant add-on with its own self-contained FSD:
**[swisssolarforecast-fsd.md](../swisssolarforecast/Documents/swisssolarforecast-fsd.md)**.

EnergyManager consumes its output as the PV input to optimization — the `pv_forecast` bucket
(measurement `pv_forecast`, tag `inverter="total"`, fields `power_w_p50` / `energy_wh_p50` and the
`p10`/`p90` bands, at 15-minute resolution). See the SwissSolarForecast FSD for the full output
schema, the MeteoSwiss ICON / STAC pipeline, PV-system configuration, and shading correction.
